...
export const getAntiEnabledMessage = (feature) => `${feature} has been enabled.`;
export const getAntiDisabledMessage = (feature) => `${feature} has been disabled.`;
export const getAntiActionSetMessage = (feature, action) => `action for ${feature} set to: ${action}`;
export const getBotJoinMessage = (prefix) => `bot is ready. type ${prefix}help to get started.`;
export const getBotLeaveMessage = () => `bot is leaving the group. farewell!`;

// ============ MESSAGE COMMANDS ============
export const messageCommandsList = [
    {
        name: 'messages',
        description: 'view all message categories',
        category: 'owner',
        ownerOnly: true,
        async execute(sock, msg, args, prefix, config) {
            const chatId = msg.key.remoteJid;
            const categories = Object.keys(messages);
            let text = `📋 message categories\n\n`;
    ...
    },
    {
        ...
        async execute(sock, msg, args, prefix, config) {
            ...
            saveMessages();
            await sock.sendMessage(chatId, { text: `✅ message ${index + 1} in "${category}" updated.` }, { quoted: msg });
        }
    },
    {
        name: 'addmsg',
