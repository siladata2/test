...
export default {
    ...
    async execute(sock, msg, args, prefix, config) {
        ...
        if (!targetJid) {
            ...
            const message = `*╭┈┈┄⊰ ${styledName} - UNBLOCK USER ⊱┄┄┄◈*
┋
┋ •> 📋 *Usage:*
┋ •> ${prefix}unblock @user
┋ •> ${prefix}unblock <number>
┋ •> Reply to user's message with ${prefix}unblock
┋
╰┄┄┄┄┄┈┈┈┈┄┄┄◈
> ® ${config.POWERED_BY}`;
        ...
        try {
            ...
            if (config.blockedUsersManager) {
                config.blockedUsersManager.remove(targetJid);
            }
            
            const styledName = config.applyFont(config.BOT_NAME, config.BOT_FONT);
            const message = `*╭┈┈┄⊰ ${styledName} - USER UNBLOCKED ⊱┄┄┄◈*
┋
...
> ® ${config.POWERED_BY}`;
            ...
            const errorMsg = `❌ *Failed to unblock user!*\nError: ${error.message}`;
            if (config.sendStyledMessage) {
