...
export default {
    name: 'reject',
    description: 'Reject a join request from a user',
    category: 'group',
    alias: ['deny', 'rejectrequest', 'joinreject'],
    ...
    async execute(sock, msg, args, prefix, config) {
        ...
        if (!adminStatus.isSenderAdmin && !isOwner) {
            const errorMsg = `❌ *Only group admins can reject join requests!*`;
            if (config.sendStyledMessage) {
                await config.sendStyledMessage(sock, chatId, errorMsg, { quoted: msg });
            } else {
                await sock.sendMessage(chatId, { text: errorMsg }, { quoted: msg });
        ...
        if (!targetJid) {
            ...
            const message = `*╭┈┈┄⊰ ${styledName} - REJECT REQUEST ⊱┄┄┄◈*
┋
┋ •> 📋 *Usage:*
┋ •> ${prefix}reject @user
┋ •> ${prefix}reject <number>
┋ •> Reply to user's message with ${prefix}reject
┋
╰┄┄┄┄┄┈┈┈┈┄┄┄◈
> ® ${config.POWERED_BY}`;
        ...
        try {
            // Reject request (can't directly reject, so we just don't add)
            try {
            ...
            const styledName = config.applyFont(config.BOT_NAME, config.BOT_FONT);
            const message = `*╭┈┈┄⊰ ${styledName} - REQUEST REJECTED ⊱┄┄┄◈*
┋
┋ •> ❌ @${targetName}'s join request has been REJECTED!
┋ •> 👤 Rejected by: @${senderJid.split('@')[0]}
┋
╰┄┄┄┄┄┈┈┈┈┄┄┄◈
> ® ${config.POWERED_BY}`;
            ...
            const errorMsg = `❌ *Failed to reject request!*\nError: ${error.message}`;
            if (config.sendStyledMessage) {
                await config.sendStyledMessage(sock, chatId, errorMsg, { quoted: msg });
            } else {
                await sock.sendMessage(chatId, { text: errorMsg }, { quoted: msg });
