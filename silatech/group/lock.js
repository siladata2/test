// ============================================
// LOCK COMMAND - Lock group (only admins can send)
...
export default {
    ...
    async execute(sock, msg, args, prefix, config) {
        ...
        if (!adminStatus.isSenderAdmin && !isOwner) {
            const errorMsg = `❌ *Only group admins can lock the group!*`;
            if (config.sendStyledMessage) {
                await config.sendStyledMessage(sock, chatId, errorMsg, { quoted: msg });
            } else {
                await sock.sendMessage(chatId, { text: errorMsg }, { quoted: msg });
        ...
        if (!adminStatus.isBotAdmin) {
            const errorMsg = `❌ *Bot needs to be admin to lock the group!*`;
            if (config.sendStyledMessage) {
                await config.sendStyledMessage(sock, chatId, errorMsg, { quoted: msg });
            } else {
                await sock.sendMessage(chatId, { text: errorMsg }, { quoted: msg });
        ...
        try {
            ...
            const errorMsg = `❌ *Failed to lock group!*\nError: ${error.message}`;
            if (config.sendStyledMessage) {
                await config.sendStyledMessage(sock, chatId, errorMsg, { quoted: msg });
            } else {
                await sock.sendMessage(chatId, { text: errorMsg }, { quoted: msg });
